...
import { JwtTokenPayloadDto } from '../auth/dto/jwt.token.payload.dto';
import { GqlExecutionContext } from '@nestjs/graphql';

export const User = createParamDecorator(
  (
    data: keyof JwtTokenPayloadDto,
    context: ExecutionContext & { contextType: string },
  ): string | bigint | JwtTokenPayloadDto => {
    let request: { user: JwtTokenPayloadDto };
